...
export class DaftarBeritaPage implements OnInit {
  kategoriId: string = '';
  beritaList: any[] = []; // Ini akan menampung berita yang sudah difilter
  ...
  ngOnInit() {
    // Baris ini akan mengambil ID dari URL, contoh: 'economy'
    // Jika tidak ada ID di URL (misal dari menu bar), nilainya akan jadi string kosong ''
    this.kategoriId = this.route.snapshot.paramMap.get('categoryId') || '';
    
    // Memanggil fungsi filter untuk memproses berita
    this.filterBerita();
  }

  // Fungsi ini sudah benar.
  // Ia akan memfilter jika 'kategoriId' ada isinya,
  // atau menampilkan semua berita jika 'kategoriId' kosong.
  filterBerita() {
    if (this.kategoriId) {
      this.beritaList = this.semuaBerita.filter(b => b.kategori.includes(this.kategoriId));
    } else {
      // Jika tidak ada kategoriId (diakses dari menu bar), tampilkan semua berita
